...
本节点暂不接入：Ethernet/LAN8720A、W25Q128、电机和 ESP8266。CAN1 已加入静默内部回环诊断；这只验证 MCU 内部控制器、FIFO 和中断链路，外部 CANH/CANL 仍需独立验收。

## 节点 7 CAN 原理图合同

| 功能 | 网络/引脚 | 当前结论 |
| --- | --- | --- |
| CAN 控制器 | CAN1 | 原理图 CPU 表与 CAN 区域一致 |
| CAN1_RX | PA11 | P9 跳帽接向 CAN 侧时，`PA11` 与 `CAN_RX` 相连 |
| CAN1_TX | PA12 | P9 跳帽接向 CAN 侧时，`PA12` 与 `CAN_TX` 相连 |
| 收发器 | U8 TJA1040 | 5 V 收发器，尚未实机启用 |
| 总线端子 | P7 CANH/CANL | 外部 CAN 节点接口 |
| 终端 | R18 120 Ω | 是否保留取决于总线拓扑 |

P9 是 CAN/USB 复用选择器，不是固定连线：中间脚为 PA11/PA12，一侧连接 U8 的 CAN_RX/CAN_TX，另一侧通过 R3/R2 连接 USB D-/D+。CAN 测试前，两只跳帽都应接向 U8/CAN 一侧。
